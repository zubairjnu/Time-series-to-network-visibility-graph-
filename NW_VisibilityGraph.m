...
% pause
save('Aij' ,'A')
dlmwrite('adjacency_matrix.dat',A,'-append')
sz_size_y = size(y);
M = sz_size_y(1,1);
for i=1:M
    for k=1:M
    ...
    B=A';
    degree_of_node = sum(B==1);
    degree_dist = unique(degree_of_node);
    b=size(degree_dist);
    n = b(2);
    
    for j = 1:n
        degree(j) = degree_dist(j);
        distrib(j) = sum(degree_of_node == degree(j));
        prob_distrib(j) = distrib(j)/M;
    end
    
    dlmwrite('pk.dat',prob_distrib,'-append')
%end    
%size degree
%size probability_distrib
p_k = [prob_distrib;degree];
pk=p_k(:)
save('p_k' ,'pk')
%    prob_distrib; dlmwrite('distributions.txt',prob_distrib,'-append')
    mean(prob_distrib);
    a = dlmread('distributions.txt'); 
    mean_val = mean(a);
 plot(mean_val,'xr')
%	'Markeredgecolor', 'k',2,...
%	 'MarkerFacecolor','g', ...
%	 'markerSize', 10)
%title('Degree Distribution of random Network')
%xlable('Degree')    
% pause            
